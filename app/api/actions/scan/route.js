...
// Defaults (can be overridden via ?timeout=NNNNN)
const DEFAULT_HARD_LIMIT_MS = 20000;  // 20s overall
const DEFAULT_PAGE_TIMEOUT_MS = 14000;
const DEFAULT_CSS_TIMEOUT_MS  = 2500;
...
export async function GET(req) {
  ...
  // allow override: ?timeout=25000 (clamped 5s..28s)
  const userBudget = parseInt(searchParams.get("timeout") || "", 10);
  const HARD_LIMIT_MS = clamp(Number.isFinite(userBudget) ? userBudget : DEFAULT_HARD_LIMIT_MS, 5000, 28000);
  ...
  try {
    const html = await fetchText(target.href, Math.min(DEFAULT_PAGE_TIMEOUT_MS, timeLeft()));
    ...
    const list = [...images];
    return Response.json({ pageUrl: target.href, count: list.length, images: list, mode: "edge" }, {
      headers: { "cache-control": "no-store" }
    });
  } catch (e) {
    return Response.json({ error: e.message || "timeout", mode: "edge" }, { status: 502 });
  }
}
